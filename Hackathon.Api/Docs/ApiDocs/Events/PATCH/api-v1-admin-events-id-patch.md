...
| `status` | `int` | Không | Trạng thái của event. Giá trị: `0` (Draft), `1` (Published), `2` (Closed). Nếu không truyền sẽ giữ nguyên trạng thái hiện tại. |
...
- `status` nếu truyền được bind trực tiếp vào `EventStatusEnum` (`Draft`, `Published`, `Closed`). Nếu không truyền hoặc truyền `null`, sẽ giữ nguyên trạng thái hiện tại của event.
