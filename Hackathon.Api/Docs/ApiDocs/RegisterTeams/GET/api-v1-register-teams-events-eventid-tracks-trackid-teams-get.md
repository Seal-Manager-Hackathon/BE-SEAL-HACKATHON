# Xem danh sách đội thi theo track trong event

## Tác dụng
API dành cho Staff, Lecturer hoặc Admin (đã được phân công trong event) xem danh sách các đội thi đã đăng ký trong một track cụ thể của event, kèm thông tin trạng thái thi đấu (đã bị loại hay chưa), vòng thi hiện tại, và hỗ trợ tìm kiếm/lọc.
...
| `trackId` | `guid` | Có | Id của track cần lấy danh sách đội thi. |

## Query parameters
| Tên | Kiểu dữ liệu | Bắt buộc | Mô tả |
|---|---|---|---:|---|
| `keyword` | `string` | Không | Từ khóa tìm kiếm theo tên đội (không phân biệt hoa thường). |
| `status` | `int` | Không | Lọc theo trạng thái đơn đăng ký. Giá trị: `0` (Pending), `1` (Approved), `2` (Rejected). Nếu không truyền thì lấy tất cả. |
...
## Ví dụ request
```http
GET /api/v1/register-teams/events/00000000-0000-0000-0000-000000000000/tracks/11111111-1111-1111-1111-111111111111/teams?keyword=abc&status=1&isEliminated=false
  ...
  "data": [
    {
      ...
      "teamId": "guid",
      "teamName": "string",
      "status": 1, /* RegisterTeamStatusEnum: 0=Pending, 1=Approved, 2=Rejected */
      "topicId": "guid|null",
...
- `eventId` và `trackId` là bắt buộc trên path.
- Event phải tồn tại và chưa bị disable, nếu không trả `EVENT_NOT_FOUND`.
- Track phải tồn tại, thuộc event đó và chưa bị disable, nếu không trả `TRACK_NOT_FOUND`.
...
- Method: `GetTeamsByTrack(Guid eventId, Guid trackId, Request.GetTeamsByTrackRequest request)`.
- Endpoint dùng route `GET /api/v1/register-teams/events/{eventId:guid}/tracks/{trackId:guid}/teams` và `StaffLecturerOrAdminPolicy`.
