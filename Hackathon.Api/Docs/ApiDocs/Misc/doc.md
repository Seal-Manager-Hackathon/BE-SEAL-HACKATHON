...
- [`POST /api/v1/judge/scores/{scoreId}/retake`](Judge/POST/api-v1-judge-scores-scoreId-retake-post.md) — Judge chấm lại điểm phúc khảo từ score gốc của chính mình. Quyền: Judge owner score gốc. Entity: `Scores.IsRetake = true` + `Scores.RetakeFromScoreId`. Lý do: Lưu điểm phúc khảo riêng biệt (BR-SCO-04, BR-REP-06).
...
- [`GET /api/v1/judge/submissions/regrade`](Judge/GET/api-v1-judge-submissions-regrade-get.md) — Judge xem danh sách bài phúc khảo cần chấm lại dựa trên score gốc của chính mình. Quyền: Judge owner score gốc. Entity: `Submissions` + `Reports` + `Scores`. Filter: `eventId`, `trackId`, `isRegraded`.
...
- [`GET /api/v1/staff/reports`](Staff/GET/api-v1-staff-reports-get.md) — Staff/Admin xem danh sách các báo cáo/khiếu nại/yêu cầu hỗ trợ trong hệ thống. Quyền: Staff/Admin. Entity: `Reports`. Filter: `status`, `typeReport`, `eventId`. Lý do: Quản lý các case khiếu nại.
- [`GET /api/v1/staff/reports/{reportId}`](Staff/GET/api-v1-staff-reports-reportId-get.md) — Staff xem chi tiết khiếu nại (bao gồm link file, ảnh minh chứng, bài nộp đính kèm). Quyền: Staff/Admin. Entity: `Reports`.
- [`PATCH /api/v1/staff/reports/{reportId}/status`](Staff/PATCH/api-v1-staff-reports-reportId-status-patch.md) — Staff cập nhật trạng thái xử lý report (`Open/Closed`) kèm lý do phản hồi. Quyền: Staff/Admin. Entity: `Reports.Status` + `Reports.Reason`. Lý do: Đóng/Mở khiếu nại (BR-REP-05).
- [`POST /api/v1/staff/reports/{reportId}/regrade`](Staff/POST/api-v1-staff-reports-reportId-regrade-post.md) — BTC phê duyệt cho phép chấm lại bài thi, set `Reports.Status = Approved` và `Submissions.IsRegrade = true`. Quyền: Staff/Admin. Entity: `Reports` + `Submissions`. Lý do: BTC phê chuẩn phúc khảo; judge chấm lại là judge đã chấm score gốc.
- [`POST /api/v1/teams/{teamId}/submissions/{submissionId}/appeal`](Teams/POST/POST-api-v1-teams-teamId-submissions-submissionId-appeal.md) — Team leader gửi khiếu nại/phúc khảo cho bài nộp đã có kết quả; request truyền `submissionId` để Staff/Admin xem đúng bài và duyệt regrade nếu cần. Quyền: Authenticated (Leader). Entity: `Reports` + `Submissions`.
- [`GET /api/v1/staff/submissions/regrade`](Staff/GET/api-v1-staff-submissions-regrade-get.md) — Staff/Admin xem danh sách bài nộp đã duyệt regrade, kèm tiến độ score gốc đã được chấm lại chưa (`PendingRegrade`, `PartiallyRegraded`, `RegradeCompleted`). Quyền: Staff/Admin. Entity: `Submissions` + `Reports` + `Scores`. Filter: `eventId`, `trackId`, `regradeStatus`.
