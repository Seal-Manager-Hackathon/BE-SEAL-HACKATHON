...
| `keyword` | `string` | Không | Từ khóa tìm kiếm theo tên hoặc mô tả track. |
...
## Ví dụ request
```http
GET /api/tracks?eventId=00000000-0000-0000-0000-000000000000&keyword=ai&isDisable=false&pageIndex=1&pageSize=10
```
...
- Nếu truyền `keyword`, tìm kiếm không phân biệt hoa thường theo tên hoặc mô tả track.
- Track thuộc event bị disable không nên được trả về, trừ khi nghiệp vụ cho phép lấy dữ liệu disable bằng query riêng.
- Kết quả nên sắp xếp theo `Name` tăng dần.
